...
- 结构库恢复简笔示意图，支持自定义模板与变量方案的批量删除、确认和撤回；保留已有实例。
- 布尔格采用4:3比例和补集对称布局，统一圆形接口、箭头尖端和节点边界；去除LMN空节点重复内容条。
- 全局设计优先于单个对象；导航条目改为有层次的卡片；导入默认自动识别，格式选择折叠保留。
- 并行预加载模块、当前版本静态缓存、共享数据库连接及导航索引，导出预览按需生成；同版本重开不重复迁移写库。
- 清理17个过时文件及废弃导航/删除实现，保持LKL1/2/3导入和有效回归测试。


## V5.0.0 — 2026-09-06

- 发布目标统一为网页，增加浏览器验收 CI，并在 Pages 部署前执行测试。
- 增加共享图片/plot 正文编辑、线性代数、数学结构模型与能力目录。
- 修复关系标签叠放、实际背景测量、自环、绘图范围与浮动面板遮挡。
- 重构有限函数、交换图、商集/划分、笛卡尔积、置换、二面体群、离散动力系统、流网络及相关树/格模型；增加 n 元中心。
- 增加完整 LKL 3 档案与事务导入，隔离副本外部身份，保留旧实例定义，重写可阅读手册。
- 验证：200 项单元/回归测试；浏览器覆盖 12 个结构、标签、两种图片输入、绘图、矩阵、完整文件往返、数据库重载与手册可见性。
