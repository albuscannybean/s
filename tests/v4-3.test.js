...
test('V4.3 UI exposes tracked libraries, three search modes, command-only top bar and unified operations',()=>{
  ...
  assert.match(html,/V5\.1\.1/);assert.match(html,/id="searchWorkbench"/);assert.match(html,/>命令行</);assert.match(html,/id="insertStructure" class="primary hidden"/);
  for(const label of['结构检索','全文检索','LKL 检索','＋ 新建结构','操作','代数','几何'])assert.match(controller,new RegExp(label));
  assert.match(controller,/renderKnowledgeNavigator/);assert.match(controller,/renderNavigatorSlot/);assert.match(controller,/objectVisibility/);assert.match(controller,/camera\.projection/);
