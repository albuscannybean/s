...
test('HF4 ships global Home navigation, unified search routing, resilient hit tests, and fresh cache',async()=>{
  ...
  assert.match(renderer,/coordinate-motion-hit/);assert.match(renderer,/geometry-line-hit/);assert.match(styles,/\.coordinate-plot-hit\{[^}]*stroke-width:16/);assert.match(metadata,/APP_HOTFIX='V4\.3\.0-HF4'/);assert.match(worker,/lmn-v4\.3\.0-stable-20260903-10/);assert.match(worker,/geometry\/geometry-operands\.js/);
});
