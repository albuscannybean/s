...
test('HF4 ships global Home navigation, unified search routing, resilient hit tests, and fresh cache',async()=>{
  ...
  assert.match(renderer,/coordinate-motion-hit/);assert.match(renderer,/geometry-line-hit/);assert.match(styles,/\.coordinate-plot-hit\{[^}]*stroke-width:16/);assert.match(metadata,/APP_HOTFIX='V5\.1\.1'/);assert.match(worker,/lmn-v5\.1\.1-web-[a-f0-9]{12}/);assert.match(worker,/geometry\/geometry-operands\.js/);
});
