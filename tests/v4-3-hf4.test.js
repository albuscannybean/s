...
test('HF4 ships global Home navigation, unified search routing, resilient hit tests, and fresh cache',async()=>{
  ...
  assert.match(controller,/renderKnowledgeNavigator\(root,\{index/);assert.doesNotMatch(controller,/if\(!this\.knowledge\)\{root\.innerHTML='<p class="nav-empty">新建知识后/);assert.match(controller,/openSearchResult\(target/);assert.match(controller,/elementsFromPoint/);assert.match(controller,/distance<=14/);
  assert.match(renderer,/coordinate-motion-hit/);assert.match(renderer,/geometry-line-hit/);assert.match(styles,/\.coordinate-plot-hit\{[^}]*stroke-width:16/);assert.match(metadata,/APP_HOTFIX='V5\.1\.3'/);assert.match(worker,/lmn-v5\.1\.3-web-[a-f0-9]{12}/);assert.match(worker,/geometry\/geometry-operands\.js/);
});
