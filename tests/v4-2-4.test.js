...
test('V4.2.4 UI exposes settings, design, zoomable preview, timeline wiring and knowledge deletion',()=>{
  ...
  assert.match(html,/V5\.1\.3/);assert.match(html,/id="sourcePreviewZoom"/);assert.match(html,/内容库/);
  assert.match(controller,/\['settings','设置'\]/);assert.match(controller,/\['design','设计'\]/);
  assert.match(controller,/addTimelineNode/);assert.match(controller,/label:t\('演化'\)/);assert.match(controller,/visual:\{labelPosition:'center'\}/);
