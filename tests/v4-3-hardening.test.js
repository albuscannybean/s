...
test('RC completion uses the native scene for query nesting and keeps camera/theme controls live',()=>{
  ...
  assert.match(controller,/ensureStructureView\(this\.instance\)\.camera\[key\]=sync/);assert.match(controller,/navigatorPathSignature===signature/);assert.match(controller,/ensureCurrentPathExpanded/);
...
test('Query AST matches direction, nested structure and returns exact highlights',()=>{
  const state=baseState(),rootTemplate=getBuiltinTemplate('builtin:lmn-444'),childTemplate=getBuiltinTemplate('builtin:directed-graph'),root=createStructureInstance(rootTemplate,'knowledge'),child=createStructureInstance(childTemplate,'knowledge');state.structureInstances.push(root,child);bindTarget(root,rootTemplate,'M2','structure',child.id,{placementMode:'construct'});
...
test('explicit LKL placements materialize stable navigator bindings',()=>{
  const state=baseState(),knowledge={id:'placement-root',title:'Root'},parentTemplate=getBuiltinTemplate('builtin:lmn-444'),childTemplate=getBuiltinTemplate('builtin:directed-graph'),parent=createStructureInstance(parentTemplate,knowledge.id),child=createStructureInstance(childTemplate,knowledge.id);state.knowledge.push(knowledge);state.structureInstances.push(parent,child);bindTarget(parent,parentTemplate,'M2','structure',child.id,{placementMode:'reference'});
  const model=exportStateKnowledgePackage(state,{rootKnowledgeId:knowledge.id,packageId:'pkg:placements'}),parentModel=model.structureInstances.find(item=>item.templateRef==='builtin:lmn-444'),container=parentModel.containers.find(item=>item.slotId==='M2');container.structureRefs=[];
  const parsed=importKnowledgePackage(serializeKnowledgePackage(model));assert.equal(parsed.valid,true,parsed.errors?.[0]?.message);const plan=buildImportPlan(parsed.package,baseState()),restoredParent=plan.nextState.structureInstances.find(item=>item.templateId==='builtin:lmn-444'),restoredChild=restoredParent.containers.M2.children.find(item=>item.type==='structure');
...
test('offline cache includes every V4.3 controller dependency',()=>{
  const worker=fs.readFileSync(new URL('../apps/web/sw.js',import.meta.url),'utf8');assert.match(worker,/lmn-v5\.1\.4-web-[a-f0-9]{12}/);for(const asset of['packages/app-metadata.js','packages/ui/localization.js','packages/ui/command-registry.js','packages/navigation/document-id.js','packages/navigation/tab-session.js','packages/geometry/geometry-operands.js','packages/structure-engine/board.js','packages/search-engine/query-engine.js','packages/cognitive-runtime/task-context.js','packages/cognitive-runtime/semantic-index.js','packages/cognitive-runtime/activation.js','packages/cognitive-runtime/cognitive-plan.js','packages/cognitive-runtime/role-inference.js','packages/cognitive-runtime/topology.js','packages/cognitive-runtime/validation.js','packages/cognitive-runtime/compiler.js','packages/cognitive-runtime/derived-projection.js','packages/lkl2/schema.js'])assert.match(worker,new RegExp(asset.replace(/[./]/g,'\\$&')),asset);
});
