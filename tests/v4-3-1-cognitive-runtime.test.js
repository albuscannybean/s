...
test('Derived Projection is ephemeral until explicit persistence and reloads as a valid instance',()=>{
  ...
  const projection=createDerivedProjection({taskContext,activation,state,projectionType:'proof'});assert.equal(projection.ephemeral,true);assert.equal(state.structureInstances.length,count);assert.equal(projection.instance.templateId,'builtin:directed-graph');assert.equal(projection.instance.parameters.topology,'dag');
