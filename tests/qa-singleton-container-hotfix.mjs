...
await page.evaluate(async()=>{
  const[{createKnowledge},{bindTarget,createStructureInstance},{setLocalDisplayTitle}]=await Promise.all([import('/packages/domain/core.js'),import('/packages/structure-engine/model.js'),import('/packages/domain/identity.js')]),app=lmnWorkspace,root=createKnowledge('康德'),child=createKnowledge('知识与自由的双重批判问题'),template=app.state.structureTemplates.find(item=>item.id==='builtin:dependency-dag'),instance=createStructureInstance(template,root.id);
