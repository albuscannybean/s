...
export function exportStateKnowledgePackage(state,{rootKnowledgeId,packageId='pkg:local-export',title=null}={}){
  ...
  for(const instance of instances){const instanceRef={type:'structure',id:instanceStable.get(instance.id)};if(knowledgeStable.has(instance.ownerKnowledgeId))pushPlacement({stableId:`owner-${instanceStable.get(instance.id)}`,target:instanceRef,parent:{type:'knowledge',id:knowledgeStable.get(instance.ownerKnowledgeId)},mode:'construct',order:0,path:''});for(const binding of instance.bindings??[]){const target=typedStable(binding.targetType,binding.targetId);pushPlacement({stableId:local(binding.externalStableId,binding.id),target,parent:instanceRef,mode:binding.metadata?.placementMode==='reference'?'reference':'construct',order:Number(binding.metadata?.order??0),path:binding.slotId})}}
