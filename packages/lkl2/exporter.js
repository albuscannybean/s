...
const normalizedType=type=>type==='structure-instance'?'structure':type;
function stableNames(records){const result=new Map(),used=new Set();for(const record of records){const preferred=local(record.externalStableId,record.stableId??record.id);let stable=preferred,suffix=2;while(used.has(stable))stable=`${preferred}-${suffix++}`;used.add(stable);result.set(record.id,stable)}return result}
...
export function exportStateKnowledgePackage(state,{rootKnowledgeId,packageId='pkg:local-export',title=null}={}){
  ...
  while(queue.length){
    ...
    const seen=current.type==='knowledge'?knowledgeSeen:current.type==='structure'?instanceSeen:current.type==='content'?contentSeen:boardSeen;
    if(seen.has(current.id))continue;
    // Addresses are persisted independently of templates. Direct Knowledge and
    // Content children must survive exporting after a file-like move.
    for(const placement of state.placements??[])if(normalizedType(placement.parentType)===current.type&&placement.parentId===current.id)queue.push({type:normalizedType(placement.targetType),id:placement.targetId});
    if(current.type==='content'){collectContent({targetId:current.id});continue}
    if(current.type==='knowledge'){
      ...
      for(const content of (state.contentObjects??[]).filter(value=>value.ownerKnowledgeId===item.id))queue.push({type:'content',id:content.id});
  ...
  const knowledgeStable=stableNames(knowledge),instanceStable=stableNames(instances),contentStable=stableNames(contents),boardStable=stableNames(boards);
  const usedTemplateIds=new Set(instances.map(item=>item.templateId).filter(id=>id&&!String(id).startsWith('builtin:'))),templates=(state.structureTemplates??[]).filter(item=>usedTemplateIds.has(item.id)),templateStable=stableNames(templates),contentRef=child=>contentStable.get(child.targetId)??contentStable.get(child.id);
  const exportedRelations=[...new Map(relations.map(item=>[item.id,item])).values()].filter(item=>knowledgeStable.has(item.sourceId)&&knowledgeStable.has(item.targetId)),relationStable=stableNames(exportedRelations),viewStable=stableNames(views);
  ...
  const placements=[],placementKeys=new Set(),placementNames=new Set(),pushPlacement=item=>{if(!item.target||!item.parent)return;const key=`${item.target.type}:${item.target.id}|${item.parent.type}:${item.parent.id}|${item.path??''}|${item.mode}`;if(placementKeys.has(key))return;placementKeys.add(key);const preferred=item.stableId;let suffix=2;while(placementNames.has(item.stableId))item.stableId=`${preferred}-${suffix++}`;placementNames.add(item.stableId);placements.push(item)};
  ...
  for(const content of contents)if(knowledgeStable.has(content.ownerKnowledgeId)&&!explicitlyPlacedTargets.has(`content:${content.id}`))pushPlacement({stableId:`owner-content-${contentStable.get(content.id)}`,target:{type:'content',id:contentStable.get(content.id)},parent:{type:'knowledge',id:knowledgeStable.get(content.ownerKnowledgeId)},mode:'construct',order:0,path:''});
  ...
  const defaultBoard=boards.find(item=>item.id===packageRecord?.defaultBoardId)??boards[0],fallbackEntries=[{stableId:'root',title:'开始阅读',target:{type:'knowledge',id:knowledgeStable.get(root.id)}},...(defaultBoard?[{stableId:'board-root',title:defaultBoard.title,target:{type:'board',id:boardStable.get(defaultBoard.id)}}]:[])];
  const entryRecords={knowledge:state.knowledge,structure:state.structureInstances,content:state.contentObjects,board:state.boards},namespace=packageRecord?.packageId??packageRecord?.stableId??packageId;
  const retainedEntries=(packageRecord?.entries??[]).flatMap(entry=>{
    const target=entry.target;if(!target)return[];const kind=normalizedType(target.type),records=entryRecords[kind]??[],record=records.find(item=>item.id===target.id||item.externalStableId===target.id)||records.find(item=>item.packageId===namespace&&local(item.externalStableId,item.stableId??item.id)===target.id),resolved=record?typedStable(kind,record.id):null;
    return resolved?[{...clone(entry),target:resolved}]:[];
  });
  const entries=retainedEntries.length?retainedEntries:fallbackEntries,preferredEntry=packageRecord?.defaultEntry??(defaultBoard?'board-root':'root'),defaultEntry=entries.some(entry=>entry.stableId===preferredEntry)?preferredEntry:entries[0].stableId,packageVariableSchemes=(state.variableSchemes??[]).filter(item=>item.packageId===(packageRecord?.stableId??packageId));
  const runtimeVariable=variable=>({stableId:local(variable.externalStableId,variable.id),ownerRef:variable.ownerStableRef,label:variable.label,displayName:variable.displayName,nameI18n:clone(variable.nameI18n),group:variable.group,order:variable.order,kind:variable.kind,type:variable.type,value:variable.value,expression:variable.formula,displayFormula:variable.displayFormula,showOnCanvas:variable.showOnCanvas});
  ...
  return{
    ...
    structureTemplates:templates.map(item=>({stableId:templateStable.get(item.id),name:item.name,nameI18n:clone(item.nameI18n),description:item.description,descriptionI18n:clone(item.descriptionI18n),layout:clone(item.layout),parameters:clone(item.parameters??[]),variables:clone(item.variables??[]),constraints:clone(item.constraints??[]),rules:clone(item.rules??[]),visual:clone(item.visual??{accent:'#557c91'}),slots:clone(item.slots??[]),edges:clone(item.edges??[])})),
    structureInstances:instances.map(instance=>({stableId:instanceStable.get(instance.id),templateRef:String(instance.templateId).startsWith('builtin:')?instance.templateId:templateStable.get(instance.templateId),ownerRef:knowledgeStable.get(instance.ownerKnowledgeId)??'',title:instance.objectContent?.title??'',parameters:clone(instance.parameters??{}),runtime:{runtimeState:clone(instance.runtimeState??{}),plotExpressions:clone(instance.plotExpressions??[]),motionPoints:clone(instance.motionPoints??[]),objectVisibility:clone(instance.objectVisibility??{}),designStyles:clone(instance.designStyles??{}),relationStyles:clone(instance.relationStyles??{})},geometries:clone(instance.geometryPrimitives??[]),variables:(instance.variables??[]).filter(variable=>variable.declarationScope!=='global').map(variable=>({stableId:variable.id,ownerRef:'',label:variable.label,displayName:variable.displayName??variable.label,nameI18n:clone(variable.nameI18n??{'zh-CN':variable.displayName??variable.label,en:variable.displayName??variable.label}),group:variable.group??'未分组',order:Number(variable.order??0),kind:variable.kind,type:variable.type,value:variable.value,expression:variable.formula,displayFormula:variable.displayFormula,showOnCanvas:variable.showOnCanvas})),containers:Object.entries(instance.containers??{}).map(([slotId,container])=>({slotId,title:container.content?.title??container.label??'',localDisplayTitle:container.localDisplayTitle??'',knowledgeRefs:(container.children??[]).filter(item=>item.persistence!=='runtime'&&item.type==='knowledge').map(item=>knowledgeStable.get(item.targetId)).filter(Boolean),contentRefs:(container.children??[]).filter(isPersistentContent).map(contentRef).filter(Boolean),structureRefs:(container.children??[]).filter(item=>item.persistence!=='runtime'&&item.type==='structure').map(item=>instanceStable.get(item.targetId)).filter(Boolean),variableRefs:(container.children??[]).filter(item=>item.persistence!=='runtime'&&item.type==='variable').map(item=>item.targetId).filter(Boolean)}))})),
    relations:exportedRelations.map(item=>({stableId:relationStable.get(item.id),from:{type:'knowledge',id:knowledgeStable.get(item.sourceId)},to:{type:'knowledge',id:knowledgeStable.get(item.targetId)},type:item.type,label:item.label,body:item.body??item.objectContent?.body??'',bodyFormat:'markdown'})),
    variables:instances.flatMap(instance=>(instance.variables??[]).filter(variable=>variable.declarationScope==='global').map(variable=>({...runtimeVariable(variable),ownerRef:instanceStable.get(instance.id)}))),
    variableSchemes:packageVariableSchemes.map(item=>({id:item.stableId??local(item.externalStableId,item.id),title:item.title,name:item.name,description:item.description,category:item.category,builtin:item.builtin,parameters:clone(item.parameters??{}),variables:clone(item.variables??[]),viewDefaults:clone(item.viewDefaults??{}),favorite:item.favorite,hidden:item.hidden,createdAt:item.createdAt,updatedAt:item.updatedAt})),
    views:views.map(view=>{const snapshot=normalizeStructureView(view),focus=view.focus?typedStable(view.focus.type,view.focus.id):null;return{stableId:viewStable.get(view.id),forRef:instanceStable.get(view.instanceId),focus,...structureViewSemanticSnapshot({structureView:snapshot}),objectVisibility:clone(view.objectVisibility??{})}}),
    ...
    placements,
    entries,
    sources:clone(packageRecord?.sources??[])
  };
}
