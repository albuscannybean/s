...
export function lklManualMarkdown(language='zh-CN'){return String(language).startsWith('en')?lkl3ManualMarkdown('en')+'\n\n# Appendix: LKL 2 syntax reference\n\n'+englishLkl2Reference():lkl3ManualMarkdown()+'\n\n# 附录：LKL 2 语法参考\n\n'+legacyLkl2Manual();}

function englishLkl2Reference(){
 const code=value=>String.fromCharCode(96)+value+String.fromCharCode(96);
 const sections=lklSchemaCatalog().map(item=>'## '+item.kind+'\n\nRequired: '+(item.required.length?item.required.map(code).join(', '):'none')+'\n\nFields: '+item.fields.map(code).join(', ')+'\n\nChild declarations: '+(item.children.length?item.children.map(value=>code(value.kind)).join(', '):'none')).join('\n\n');
 return'# LKL '+LKL_SCHEMA.version+' reference\n\nThis reference is generated from the current parser schema. LKL is a data language. Commands beginning with / are a separate command language.\n\n'+sections+'\n\n## Geometry operands\n\nUse '+code('operand slot A')+', '+code('operand motion M1')+', '+code('operand geometry line-1')+' or '+code('operand plot curve-1')+' to reference points, moving points, derived geometry, curves and surfaces. Legacy '+code('point slot A')+' and '+code('point motion M1')+' remain supported. Export and re-import retain these dependencies.\n\n## Diagnostics\n\nDiagnostics include line, column, field path, object ID and a suggested repair. Imports are transactional and enforce object-size limits.';
}
