...
export function structureSearch(criteria,state){
  ...
  return state.structureInstances.filter(instance=>{
    ...
    const resolved=materializeTemplate(template,instance.parameters);
...
export function patternSearch(pattern,state){
  ...
  for(const instance of state.structureInstances){
    ...
    const resolved=materializeTemplate(template,instance.parameters),slots=new Map(resolved.slots.map(slot=>[slot.id,slot])),bindings=new Map((instance.bindings??[]).map(binding=>[binding.slotId,binding]));
