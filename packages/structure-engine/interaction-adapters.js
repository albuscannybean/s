...
export function getStructureInteractionAdapter(template){
  ...
  if(template?.slotFactory==='directed-node-family')return directedFamilyAdapter;
  if(template?.slotFactory==='semantic:n-center')return centerFamilyAdapter;
...
const directedFamilyAdapter={...graphAdapter,id:'graph',getEdgeCapabilities:()=>mergeCapabilities({canChangeDirection:false})};
const centerFamilyAdapter={...generatedAdapter,id:'n-center',getContainerCapabilities:(slot)=>mergeCapabilities({...generatedCapabilities,canEditContent:true,canEditAppearance:true,canAddVariable:true,canMoveVisualPosition:false}),getCreateActions:()=>[{id:'add-node',label:'添加外围节点',kind:'parameters'},{id:'add-container-content',label:'添加正文',kind:'container-content'}]};
...
const vennAdapter={...generatedAdapter,id:'venn',getContainerCapabilities:()=>mergeCapabilities({...generatedCapabilities,canEditContent:true,canEditAppearance:true,canAddVariable:false}),getCreateActions:()=>[{id:'venn-add-set',label:'添加集合',kind:'parameters'},{id:'venn-remove-set',label:'移除集合',kind:'parameters'},{id:'venn-rename-set',label:'重命名集合',kind:'display'},{id:'add-container-content',label:'添加区域正文',kind:'container-content'}],prepareDefinition:(definition,instance)=>{if(definition.runtimeMetadata?.family==='venn')return definition;const labels=instance?.parameters?.setLabels??(Number(definition.layout?.sets)===3?['A','B','C']:['A','B']),name={"A-only":`${labels[0]}∖${labels[1]}`,intersection:`${labels[0]}∩${labels[1]}`,"B-only":`${labels[1]}∖${labels[0]}`,A:`${labels[0]}∖(${labels[1]}∪${labels[2]})`,B:`${labels[1]}∖(${labels[0]}∪${labels[2]})`,C:`${labels[2]}∖(${labels[0]}∪${labels[1]})`,AB:`${labels[0]}∩${labels[1]}`,AC:`${labels[0]}∩${labels[2]}`,BC:`${labels[1]}∩${labels[2]}`,ABC:labels.join('∩')};return{...definition,slots:definition.slots.map(slot=>({...slot,label:name[slot.id]??slot.label,canonicalDefaultLabel:name[slot.id]??slot.label}))}}};
