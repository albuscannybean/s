...
export function migrateV3ToV4(v3){
  const source=[3,4].includes(v3?.schemaVersion)?v3:migrateV2ToV3(v3),templates=currentTemplates(source.structureTemplates),legacyMap=new Map();
  for(const old of source.structureTemplates??[]){
    const next=templates.find(t=>t.id===old.id),isBuiltin=old.builtin||String(old.id).startsWith('builtin:');
    const oldSemantic=isBuiltin&&next?.version>=4&&Number(old.version??0)<4&&next.id!=='builtin:coordinate-plane'&&(next.slotFactory?.startsWith('semantic:')||['builtin:decision-tree','builtin:proof-tree','builtin:dependency-dag','builtin:state-machine','builtin:lattice'].includes(next.id));
    const oldDirected=isBuiltin&&next?.slotFactory==='directed-node-family'&&Number(old.version??0)<5;
