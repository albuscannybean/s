...
export function parsePlotExpression(source){
  let original=String(source??'').trim().replace(/^f\s*\(\s*x\s*,\s*y\s*\)\s*=/i,'z=').replace(/^f\s*\(\s*x\s*\)\s*=/i,'y=');
  if(!original)throw new Error('请输入表达式');
  if(PLOT_PRESETS[original.toLowerCase()]||Object.values(PLOT_PRESETS).some(item=>item.label===original))throw new Error('预设图形需要显式参数，例如 circle(cx=0, cy=0, r=3)');
  const primitive=parsePrimitive(original);if(primitive)return primitive;
  const parts=original.split(';').map(s=>s.trim()).filter(Boolean),assignments={};
  for(const part of parts){const m=part.match(/^([xyz])\s*=\s*(.+)$/i);if(m){const key=m[1].toLowerCase();if(key in assignments)throw new Error('重复坐标 '+key);assignments[key]=m[2];}else if(parts.length>1)throw new Error('每个坐标须使用 x=、y= 或 z=');}
  if(assignments.z&&!assignments.x&&!assignments.y)return{kind:'surface',source:original,expressions:{x:'u',y:'v',z:assignments.z.replace(/\bx\b/g,'u').replace(/\by\b/g,'v')},ranges:{u:[-6,6],v:[-6,6]},dimension:'3d'};
  if(assignments.x&&assignments.y){const surface=Object.values(assignments).some(s=>/\b[uv]\b/.test(s));if(surface&&!assignments.z)throw new Error('参数曲面需要 x、y、z 三个坐标');return{kind:surface?'surface':'parametric',source:original,expressions:assignments,range:[-Math.PI*2,Math.PI*2],...(surface?{ranges:{u:[-Math.PI,Math.PI],v:[-Math.PI,Math.PI]}}:{}),dimension:assignments.z?'3d':'2d'};}
  if(assignments.x||assignments.z)throw new Error('参数曲线需要 x 与 y 坐标');
  return{kind:'function',source:original,expressions:{y:assignments.y??original},range:[-6,6],dimension:'2d'};
}

function parsePrimitive(source){
  const match=source.match(/^([a-z][a-z0-9_-]*)\s*\((.*)\)$/i);if(!match)return null;const name=match[1].toLowerCase(),definition=PRIMITIVES[name];if(!definition&&FUNCTIONS[name])return null;if(!definition)throw new Error(`未知参数化图形 ${match[1]}`);const parameters={...definition.defaults},body=match[2].trim();if(body)for(const entry of splitArguments(body)){const pair=entry.trim().match(/^([a-z][a-z0-9_]*)\s*=\s*(.+)$/i);if(!pair)throw new Error(`参数“${entry.trim()}”需要使用 名称=数值`);const parameterKey=Object.keys(parameters).find(key=>key.toLowerCase()===pair[1].toLowerCase());if(!parameterKey)throw new Error(`${name} 不支持参数 ${pair[1]}`);parameters[parameterKey]=evaluateMathExpression(pair[2])}const expressions=definition.build(parameters),kind=definition.kind??'parametric',ranges=definition.rangesFrom?.(parameters)??definition.ranges;return{kind,source,expressions,range:definition.range??ranges?.u, ranges,dimension:expressions.z?'3d':'2d',primitive:{type:name,parameters}};
}

export function samplePlotExpression(source,{samples=240,range=null,ranges=null}={}){
  const parsed=parsePlotExpression(source);if(parsed.kind==='surface')return{...parsed,segments:sampleSurface({...parsed,ranges:ranges??parsed.ranges})};const[start,end]=range??parsed.range,points=[];for(let index=0;index<=samples;index++){const value=start+(end-start)*index/samples;try{const next=safePoint(evaluatePlotPoint(parsed,value)),previous=points.at(-1);if(previous&&next&&parsed.kind==='function'){const mid=(previous.x+next.x)/2;let midpoint;try{midpoint=evaluatePlotPoint(parsed,mid)}catch{midpoint=null}if(!midpoint||Math.abs(midpoint.y-(previous.y+next.y)/2)>Math.max(1,Math.abs(previous.y),Math.abs(next.y))*4)points.push(null);}points.push(next)}catch{points.push(null)}}return{...parsed,segments:splitSegments(points)};
}
