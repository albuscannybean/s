import {getStructureCapabilities} from '../structure-engine/templates.js';
...
export function compileCognitiveTask({taskContext={},cue='',domain=null,state={},index=null,maxCandidates=12,maxRelationDepth=2,projectionType=null,generateProjection=false}={}){
  ...
  return{kind:'cognitive-compilation',structureCapabilities:getStructureCapabilities(state.structureTemplates?.length?state.structureTemplates:undefined),taskContext,taskSchema:plan.taskSchema,domainResolution:plan.domainResolution,plan,activation,roles:roleInference.roles,roleInference,topology,projectionRanking,gaps:topology.gaps,validation,transferTests:plan.transferTests,selectedProjection,projection};
}
