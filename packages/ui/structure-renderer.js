...
function drawEdges(layer,scene,options){
  layer.replaceChildren();markerDefs(layer);drawColumnAxes(layer,scene);const selected=new Set(options.selectedIds??[]),occupiedLabels=[],labelLayer=svg('g');labelLayer.classList.add('relation-label-layer');
...
function createNode(node,context){
  ...
  const top=html('div','node-top'),identity=html('span','node-identity',presentation?.mode==='singleton'?contentGlyph(presentation.itemType):node.id),role=html('span','node-role',presentation?.mode==='singleton'?contentLabel(presentation.itemType):node.role),menu=html('button','node-object-menu','•••'),label=html('strong','node-label');menu.type='button';menu.title=`${displayLabel} 操作`;menu.setAttribute('aria-label',`${displayLabel} 操作`);menu.onclick=event=>{event.stopPropagation();context.onContext?.({kind:'slot',id:node.id,event})};appendInlineContent(label,displayLabel);top.append(identity,role);if(!displayOnly)top.append(menu);article.append(top,label);if(presentation?.secondaryTitle)article.append(html('span','node-semantic-title',presentation.secondaryTitle));
