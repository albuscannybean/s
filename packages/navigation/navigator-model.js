...
export function primaryStructureForKnowledge(state,knowledgeId){return orderedConstructStructuresForKnowledge(state,knowledgeId)[0]??(state.structureInstances??[]).find(item=>item.ownerKnowledgeId===knowledgeId&&item.templateId==='builtin:lmn-444')??(state.structureInstances??[]).find(item=>item.ownerKnowledgeId===knowledgeId)??null}
