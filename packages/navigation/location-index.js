...
export function buildNavigatorIndex(state={},options={}){
 ...
 const find=target=>{if(!target)return null;if(typeof target==='string')return objects.get(target)??null;const normalized={...target,kind:target.kind??target.type,id:target.id??target.itemId??(target.kind==='structure'?target.instanceId:undefined)};if(normalized.kind==='structure'){normalized.id??=normalized.instanceId;delete normalized.instanceId;}let found=objects.get(navigatorObjectKey(normalized));if(!found)found=[...objects.values()].find(e=>e.kind===normalized.kind&&e.id===normalized.id&&(normalized.instanceId==null||e.instanceId===normalized.instanceId)&&(normalized.slotId==null||e.slotId===normalized.slotId)&&(normalized.contentScope==null||e.contentScope===normalized.contentScope));return found??null;};
