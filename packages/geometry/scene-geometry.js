import {routeIndependentRelations} from './relation-routing.js';
...
export function estimateRelationLabelWidth(label){
  ...
  let width=textWidth;try{const context=globalThis.document?.createElement('canvas').getContext('2d');if(context){context.font='12px system-ui, sans-serif';width=Math.max(width,context.measureText(String(label)).width)}}catch{}
  return clamp(Math.ceil(width)+24,36,340);
}
...
export function routeEdge(source,target,style='straight',options={}){
  if(source===target||(source.id&&source.id===target.id)){
    const start={x:source.x+source.width*.28,y:source.y},end={x:source.x+source.width*.72,y:source.y},c1={x:source.x-65,y:source.y-110},c2={x:source.x+source.width+65,y:source.y-110};
    return{selfLoop:true,start,end,points:[start,c1,c2,end],path:'M '+start.x+' '+start.y+' C '+c1.x+' '+c1.y+', '+c2.x+' '+c2.y+', '+end.x+' '+end.y};
  }
...
function gridLayout(definition,options={},sizes){
  ...
  const columnGaps=Array(Math.max(0,columns-1)).fill(gapX);for(let row=0;row<rowHeights.length;row++){const rowSlots=slots.slice(row*columns,(row+1)*columns),needed=computeRowGaps(rowSlots,definition.edges,sizes,{baseGap:gapX});needed.forEach((value,index)=>columnGaps[index]=Math.max(columnGaps[index],value))}
  const xAt=col=>(options.x??96)+colWidths.slice(0,col).reduce((sum,value,index)=>sum+value+(columnGaps[index]??0),0),yAt=row=>(options.y??116)+rowHeights.slice(0,row).reduce((sum,value)=>sum+value+gapY,0);
...
function forceLayout(definition,sizes){
  ...
  // Resolve rectangle collisions after the force simulation, without a fixed-size cage.
  for(let pass=0;pass<80;pass++){let collisions=0;for(let i=0;i<slots.length;i++)for(let j=i+1;j<slots.length;j++){const a=positions.get(slots[i].id),b=positions.get(slots[j].id),sa=measured(sizes,slots[i]),sb=measured(sizes,slots[j]),dx=b.x-a.x,dy=b.y-a.y,ox=(sa.width+sb.width)/2+28-Math.abs(dx),oy=(sa.height+sb.height)/2+28-Math.abs(dy);if(ox<=0||oy<=0)continue;collisions++;if(ox<oy){const shift=(ox/2+.1)*(Math.sign(dx)||1);a.x-=shift;b.x+=shift}else{const shift=(oy/2+.1)*(Math.sign(dy)||1);a.y-=shift;b.y+=shift}}if(!collisions)break}
...
function treeLayout(definition,{sizes,layoutDesign={}}={}){
  const children=new Map(definition.slots.map(s=>[s.id,[]])),incoming=new Set(),byId=new Map(definition.slots.map(s=>[s.id,s]));
  for(const edge of definition.edges){if(children.has(edge.sourceSlotId)&&byId.has(edge.targetSlotId)&&!incoming.has(edge.targetSlotId)){children.get(edge.sourceSlotId).push(edge.targetSlotId);incoming.add(edge.targetSlotId)}}
  const positions=new Map(),active=new Set(),levelWidth=Math.max(188,...definition.slots.map(s=>measured(sizes,s).width)),gapX=layoutDesign.autoSpacing===false?Number(layoutDesign.layerGap??96):100,gapY=layoutDesign.autoSpacing===false?Number(layoutDesign.nodeGap??32):40;let cursor=112;
  const visit=(id,depth)=>{if(positions.has(id)||active.has(id))return positions.get(id)?.centerY??cursor;active.add(id);const slot=byId.get(id),size=measured(sizes,slot),ys=children.get(id).filter(child=>!active.has(child)).map(child=>visit(child,depth+1));let centerY;
   if(ys.length)centerY=(ys[0]+ys.at(-1))/2;else{centerY=cursor+size.height/2;cursor+=size.height+gapY}
   positions.set(id,{x:80+depth*(levelWidth+gapX),y:centerY-size.height/2,centerY});active.delete(id);return centerY;};
  for(const slot of definition.slots.filter(s=>!incoming.has(s.id)))visit(slot.id,0);
  for(const slot of definition.slots)if(!positions.has(slot.id))visit(slot.id,0);
  return definition.slots.map(slot=>({...slot,...measured(sizes,slot),...positions.get(slot.id),...nodeGrammar(definition,slot)}));
}

export function projectCoordinate(coordinate={},instance={}){
  const scale=Number(instance.parameters?.scale??40),center={x:500,y:360},x=Number(coordinate.x??0),y=Number(coordinate.y??0),z=Number(coordinate.z??0),view=ensureStructureView(instance),dimension=instance.parameters?.dimension??'2d',requested=view.camera?.projection??(dimension==='3d'?'free':'xOy'),projection=requested;
...
function addCoordinateBackground(items,instance,options){
  const scale=Number(instance.parameters?.scale??40),center={x:500,y:360},dimension=instance.parameters?.dimension??'2d',view=ensureStructureView(instance),requested=view.camera?.projection??(dimension==='3d'?'free':'xOy'),projection=requested,zoom=Number(options.zoom??1)||1,major=niceStep(70/(scale*zoom)),minor=major/(scale*zoom*major>125?5:2),visible=coordinateViewport(options,scale,center),margin=major*2,planeAxes=projection==='yOz'?['y','z']:projection==='xOz'?['x','z']:['x','y'],point=(a,b)=>({[planeAxes[0]]:a,[planeAxes[1]]:b});
  if(projection!=='free'){
  ...
  for(const[index,plot]of(instance.plotExpressions??[]).entries())try{if(plot.visible===false||instance.objectVisibility?.[`plot:${plot.id}`]===false)continue;const parsed=parsePlotExpression(plot.source),viewportRange=[visible.xMin-margin,visible.xMax+margin],range=plot.rangeMode==='manual'?plot.range:parsed.kind==='function'||(parsed.kind==='parametric'&&!['circle','ellipse','heart'].includes(parsed.primitive?.type))?viewportRange:plot.range,ranges=plot.rangeMode==='manual'?plot.ranges:parsed.kind==='surface'&&!['sphere','torus'].includes(parsed.primitive?.type)?{u:viewportRange,v:[visible.yMin-margin,visible.yMax+margin]}:null,quality=options.interactionQuality??'full',sampled=samplePlotExpressionCached(plot.source,{range,ranges,samples:quality==='drag'?90:240,quality}),paths=sampled.segments.map(segment=>segment.filter(Boolean).map((value,pointIndex)=>{const projected=projectCoordinate(value,instance);return`${pointIndex?'L':'M'} ${projected.x.toFixed(2)} ${projected.y.toFixed(2)}`}).join(' ')).filter(Boolean);const plotBounds=sampled.segments.flat().filter(Boolean).reduce((box,p)=>{const q=projectCoordinate(p,instance);return{xMin:Math.min(box.xMin,q.x),yMin:Math.min(box.yMin,q.y),xMax:Math.max(box.xMax,q.x),yMax:Math.max(box.yMax,q.y)}},{xMin:Infinity,yMin:Infinity,xMax:-Infinity,yMax:-Infinity});delete instance.runtimeState.errors[`plot:${plot.id}`];for(const d of paths)items.push({type:'path',className:`coordinate-curve${sampled.kind==='surface'?' coordinate-surface-mesh':''}`,plotId:plot.id,title:`${plot.label??'曲线'} · ${plot.source}`,d,fill:'none',plotBounds,stroke:plot.color??['#2f7658','#7657a8','#c8922e','#477c84'][index%4],'stroke-width':sampled.kind==='surface'?1.25:2.6,opacity:sampled.kind==='surface'?.62:1,'vector-effect':'non-scaling-stroke'})}catch(error){instance.runtimeState.errors[`plot:${plot.id}`]=error.message}
...
function applyOffsets(nodes,instance,rankAxis=null){
  const offsets=instance.layoutState?.visualOffsets??{},positions=instance.layoutState?.nodePositions??{};
  return nodes.map(node=>{const override=positions[node.id],offset=offsets[node.id]??{};return{...node,x:rankAxis==='x'?node.x:override?.x??node.x+Number(offset.x??0),y:rankAxis==='y'?node.y:override?.y??node.y+Number(offset.y??0),width:override?.width??node.width,height:override?.height??node.height}});
}
...
export function buildSceneGeometry(definition,instance={},options={}){
  ...
  const background=sceneBackground(definition,instance,options),plotBoxes=background.map(item=>item.plotBounds).filter(b=>b&&Number.isFinite(b.xMin));
  const finitePlotIds=new Set((instance.plotExpressions??[]).filter(plot=>{try{return plot.rangeMode==='manual'||['circle','ellipse','heart','sphere','torus'].includes(parsePlotExpression(plot.source).primitive?.type)}catch{return false}}).map(plot=>plot.id)),fitBoxes=background.filter(item=>finitePlotIds.has(item.plotId)).map(item=>item.plotBounds).filter(b=>b&&Number.isFinite(b.xMin));
  const fitX=Math.min(0,...nodes.map(n=>n.x),...fitBoxes.map(b=>b.xMin))-64,fitY=Math.min(0,...nodes.map(n=>n.y),...fitBoxes.map(b=>b.yMin))-64,fitRight=Math.max(960,...nodes.map(n=>n.x+n.width),...fitBoxes.map(b=>b.xMax))+64,fitBottom=Math.max(650,...nodes.map(n=>n.y+n.height),...fitBoxes.map(b=>b.yMax))+64;
  const tightPlots=layout==='coordinate'&&plotBoxes.length;
  const minX=Math.min(tightPlots?Infinity:0,...plotBoxes.map(b=>b.xMin),...nodes.map(node=>node.x),...edges.flatMap(e=>(e.points??[]).map(p=>p.x)))-64,minY=Math.min(tightPlots?Infinity:0,...plotBoxes.map(b=>b.yMin),...nodes.map(node=>node.y),...edges.flatMap(e=>(e.points??[]).map(p=>p.y)))-64,maxX=Math.max(tightPlots?-Infinity:960,...plotBoxes.map(b=>b.xMax),...nodes.map(node=>node.x+node.width),...edges.flatMap(e=>(e.points??[]).map(p=>p.x)))+64,maxY=Math.max(tightPlots?-Infinity:650,...plotBoxes.map(b=>b.yMax),...nodes.map(node=>node.y+node.height),...edges.flatMap(e=>(e.points??[]).map(p=>p.y)))+64;
  return{instanceId:instance.id,templateId:definition.id,layout,nodes,edges,fitBounds:layout==='coordinate'?{x:fitX,y:fitY,width:fitRight-fitX,height:fitBottom-fitY}:null,geometry:layout==='coordinate'?buildGeometryPrimitiveScene(instance,definition,{projectCoordinate,evaluatePlotPoint}):[],background,tokens:sceneTokens(definition,instance,nodes),bounds:{x:minX,y:minY,width:maxX-minX,height:maxY-minY},viewport:options.viewport??null};
}

export function fitScene(scene,viewport,padding=72){
  const bounds=scene.fitBounds??scene.bounds,zoom=clamp(Math.min((viewport.width-padding*2)/bounds.width,(viewport.height-padding*2)/bounds.height),.015,2.5);
  return{zoom,panX:(viewport.width-bounds.width*zoom)/2-bounds.x*zoom,panY:(viewport.height-bounds.height*zoom)/2-bounds.y*zoom};
}
